import { InflationResult } from '@/lib/types';

// BLS CPI-U annual averages (1982-1984 = 100)
// Data source: U.S. Bureau of Labor Statistics
const CPI_DATA: Record<number, number> = {
  1913: 9.9, 1914: 10.0, 1915: 10.1, 1916: 10.9, 1917: 12.8, 1918: 15.1, 1919: 17.3, 1920: 20.0,
  1921: 17.9, 1922: 16.8, 1923: 17.1, 1924: 17.1, 1925: 17.5, 1926: 17.7, 1927: 17.4, 1928: 17.1,
  1929: 17.1, 1930: 16.7, 1931: 15.2, 1932: 13.7, 1933: 13.0, 1934: 13.4, 1935: 13.7, 1936: 13.9,
  1937: 14.4, 1938: 14.1, 1939: 13.9, 1940: 14.0, 1941: 14.7, 1942: 16.3, 1943: 17.3, 1944: 17.6,
  1945: 18.0, 1946: 19.5, 1947: 22.3, 1948: 24.1, 1949: 23.8, 1950: 24.1, 1951: 26.0, 1952: 26.5,
  1953: 26.7, 1954: 26.9, 1955: 26.8, 1956: 27.2, 1957: 28.1, 1958: 28.9, 1959: 29.1, 1960: 29.6,
  1961: 29.9, 1962: 30.2, 1963: 30.6, 1964: 31.0, 1965: 31.5, 1966: 32.4, 1967: 33.4, 1968: 34.8,
  1969: 36.7, 1970: 38.8, 1971: 40.5, 1972: 41.8, 1973: 44.4, 1974: 49.3, 1975: 53.8, 1976: 56.9,
  1977: 60.6, 1978: 65.2, 1979: 72.6, 1980: 82.4, 1981: 90.9, 1982: 96.5, 1983: 99.6, 1984: 103.9,
  1985: 107.6, 1986: 109.6, 1987: 113.6, 1988: 118.3, 1989: 124.0, 1990: 130.7, 1991: 136.2, 1992: 140.3,
  1993: 144.5, 1994: 148.2, 1995: 152.4, 1996: 156.9, 1997: 160.5, 1998: 163.0, 1999: 166.6, 2000: 172.2,
  2001: 177.1, 2002: 179.9, 2003: 184.0, 2004: 188.9, 2005: 195.3, 2006: 201.6, 2007: 207.342, 2008: 215.303,
  2009: 214.537, 2010: 218.056, 2011: 224.939, 2012: 229.594, 2013: 232.957, 2014: 236.736, 2015: 237.017,
  2016: 240.007, 2017: 245.120, 2018: 251.107, 2019: 255.657, 2020: 258.811, 2021: 270.970, 2022: 292.655,
  2023: 304.702, 2024: 313.689, 2025: 322.0,
};

export function calculateInflation(
  amount: number,
  fromYear: number,
  toYear: number
): InflationResult {
  const fromCPI = CPI_DATA[fromYear];
  const toCPI = CPI_DATA[toYear];

  if (fromCPI === undefined || toCPI === undefined) {
    throw new Error('Year data not available. Please select a year between 1913 and 2025.');
  }

  const multiplier = toCPI / fromCPI;
  const adjustedValue = amount * multiplier;
  const years = Math.abs(toYear - fromYear);
  const annualizedRate = years > 0 ? Math.pow(multiplier, 1 / years) - 1 : 0;

  return {
    adjustedValue: Math.round(adjustedValue * 100) / 100,
    multiplier: Math.round(multiplier * 10000) / 10000,
    annualizedRate: Math.round(annualizedRate * 10000) / 100,
  };
}

export function getAvailableYears(): number[] {
  return Object.keys(CPI_DATA)
    .map(Number)
    .sort((a, b) => a - b);
}

export function generateInflationFAQs() {
  return [
    {
      question: 'What is the Consumer Price Index (CPI)?',
      answer:
        'The CPI measures the average change in prices over time that consumers pay for a basket of goods and services (food, housing, transportation, healthcare, etc.). It is the most widely used measure of inflation, published monthly by the U.S. Bureau of Labor Statistics.',
    },
    {
      question: 'How accurate is this calculator?',
      answer:
        'This calculator uses official BLS CPI-U (Consumer Price Index for All Urban Consumers) annual average data from 1913 to present. Results are historically accurate for the periods shown. Note that CPI methodology has changed over time, and past inflation does not predict future inflation.',
    },
    {
      question: 'Why does $1 today not buy what it used to?',
      answer:
        'Inflation erodes purchasing power over time. If inflation averages 3% per year, $100 today will only buy about $74 worth of goods in 10 years. This is why keeping money in a low-interest savings account actually loses value in real terms.',
    },
    {
      question: 'How can I protect against inflation?',
      answer:
        'Investing in assets that historically outpace inflation — such as stocks, real estate, and Treasury Inflation-Protected Securities (TIPS) — helps preserve purchasing power. Historically, the S&P 500 has returned about 10% annually before inflation, or about 7% after inflation.',
    },
    {
      question: 'What was the highest inflation period in US history?',
      answer:
        'The highest sustained inflation in modern US history was the late 1970s and early 1980s, with CPI increases of 11.3% (1979), 13.5% (1980), and 10.3% (1981). Use this calculator to see just how dramatically prices changed during that period.',
    },
  ];
}
