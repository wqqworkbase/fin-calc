            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-100 p-5 md:p-7 mb-6">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="h-64"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="year" tick={{ fontSize: 12, fill: '#999' }} /><YAxis tickFormatter={(v: number | string) => `$${(Number(v) / 1000).toFixed(0)}k`} tick={{ fontSize: 12, fill: '#999' }} /><Tooltip formatter={(v: number | string) => `$${Number(v).toLocaleString()}`} /><Legend /><Line type="monotone" dataKey="balance" stroke="#059669" name="Balance" strokeWidth={2} dot={false} /><Line type="monotone" dataKey="contributions" stroke="#d1d5db" name="Contributions" strokeWidth={1} dot={false} /></LineChart></ResponsiveContainer></div>
